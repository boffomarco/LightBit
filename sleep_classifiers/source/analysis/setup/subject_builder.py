...
class SubjectBuilder(object):
    ...
    def build(subject_id):
        ...
        ax.plot(range(len(feature_cosine)), feature_cosine, label="Clock proxy")
        ax.legend()
        ...
        ax.legend()
        #
        plt.savefig(str(Constants.FIGURE_FILE_PATH.joinpath(subject_id + '_info.png')))
        plt.close()
